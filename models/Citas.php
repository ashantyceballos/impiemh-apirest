<?php
    ...
    class Cita extends Conectar {
        ...
        public function get_cita(){
            ...
            parent::set_names();
            $sql = "SELECT id,dia,hora,tipo,psicologa,nombrep,id_paciente,tema FROM citas WHERE estado=1";
        ...
        public function get_cita_id($id_cita){
            $conectar = parent::Conexion();
            parent::set_names();
            $sql = "SELECT id,dia,hora,tipo,psicologa,nombrep,id_paciente,tema FROM citas WHERE estado=1 AND id=?";
            $sql = $conectar -> prepare($sql);
        ...
        public function insert_cita($dia, $hora, $tipo, $psicologa, $nombrep, $id_paciente, $tema){
            $conectar = parent::Conexion();
            parent::set_names();
            $sql = "INSERT INTO citas (id,dia,hora,tipo,psicologa,nombrep,id_paciente,tema) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?)";
            $sql = $conectar -> prepare($sql);
            $sql -> bindValue(1, $dia);
            ...
            $sql -> bindValue(7, $tema);
            $sql -> execute();
            return $resultado = $sql -> fetchAll(PDO::FETCH_ASSOC);
        }

        //Actualizar los datos de una cita
        public function update_cita($id, $dia, $hora, $tipo, $psicologa, $nombrep, $id_paciente,$tema){
            $conectar = parent::Conexion();
            parent::set_names();
            $sql = "UPDATE citas SET dia=?,hora=?,tipo=?,psicologa=?,nombrep=?,id_paciente=?,tema=? WHERE id = ?";
            $sql = $conectar -> prepare($sql);
            $sql -> bindValue(1, $dia);
            $sql -> bindValue(2, $hora);
            $sql -> bindValue(3, $tipo);
